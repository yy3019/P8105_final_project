...
The fragility index involves iteratively calculating Fisher's exact test. After each calculation of a 2x2 table, we would artificially "add an event" to the smaller of the two groups and recalculate the Fisher's exact test. The FI is the number of iterations necessary for the test to move past your predetermined critical value, typically 0.05.

Smaller fragility indexes are not good, and suggest that the trial is more fragile.

Our study is inspired mainly by three recent articles: 
...
After all of the above, we finally have a dataframe that has just enough data to get a preliminary look at the fragility index. Our final dataset contains `r nrow(clean_final_df)` trials with `r ncol(clean_final_df)-1` relevant variables:
